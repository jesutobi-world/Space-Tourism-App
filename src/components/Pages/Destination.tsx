...
export const Destination: React.FC = () => {
  const [selectedDestination, setSelectedDestination] = useState<
    Destination
  >(data.destinations[0]);
  const [selectedLink, setSelectedLink] = useState(data.destinations[0].name);

  const handleNavClick = (destination: Destination) => {
    setSelectedDestination(destination);
    setSelectedLink(destination.name);
  };
          ...
          <AnimatePresence>
            <motion.div className="text-content" variants={textVariants}>
              <nav className="nav">
                {data.destinations.map(destination => (
                  <p
                    ...
                    className={`link ${destination.name === selectedLink ? "active" : ""}`}
              ...
              <div className="description">
                ...
                <motion.div className="intro" variants={textVariants}>
